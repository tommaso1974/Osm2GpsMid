...
import osmToGpsMid.route.Location;
import osmToGpsMid.route.LocationTableModel;
import osmToGpsMid.route.Route;
...
public class GuiConfigWizard extends JFrame implements Runnable, ActionListener, SelectionListener {
    ...
    /**
     * Takes the route destinations from the config object and puts them on the
     * map.
     */
    ...
    /**
     * Finds all files in the Osm2GpsMid JAR that match the pattern
     * "GpsMid-*.jar" or "GpsMid-*.apk" and puts their names in a vector,
     * cutting off at the last "-". E.g. GpsMid-Generic-full-0.6.4-map65.jar ->
     * GpsMid-Generic-full
     *
    ...
    /**
     * Finds all files in the Osm2GpsMid JAR that match the pattern
     * "*.properties" and puts their names in a vector, without the
     * ".properties". E.g. Cologne.properties -> Cologne
     *
     * @return Vector containing the names
     */
    ...
    /**
     * Opens a file chooser dialog for the style file. Lets the config read the
     * file when it was chosen.
     */
    ...
    /**
     * Opens a file chooser dialog for the bundle .properties file. Lets the
     * config read the file when it was chosen.
     */
    ...
    /**
     * Writes the current properties to a .properties file TODO: Shouldn't
     * useCellID be written too? TODO: Add useHouseNumbers and useWordSearch And
     * what about the cellSource variable from Configuration.java?
     *
    ...
    private void handleCalculateRoute() {
        if (config.getRouteList().size() > 1) {
            ...
            JOptionPane.showMessageDialog(this, "Please add first at least two route destinations with Alt+Click or Shift+Click on the map", "Route Corridor Calculation", JOptionPane.ERROR_MESSAGE);
        }
    ...
    /**
     * Used to reenable the "Close" button from BundleGpsMid (after Midlet
     * creation).
     */
