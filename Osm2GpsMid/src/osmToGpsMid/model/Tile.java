 ...
 *
 * Copyright (C) 2007 Harald Mueller Copyright (C) 2007 Kai Krueger
 *
 */
...
import osmToGpsMid.CreateGpsMidData;
...
public class Tile {
    ...
    /**
     * Center latitude of the tile in degrees. This is used as a reference point
     * for the relative coordinates stored in the file.
     */
    public float centerLat;

    /**
     * Center longitude of the tile in degrees. This is used as a reference
     * point for the relative coordinates stored in the file.
     */
    public float centerLon;

    /**
     * If this tile is a container, this holds the reference to the first
     * sub-tile.
     */
    public Tile t1 = null;

    /**
     * If this tile is a container, this holds the reference to the second
     * sub-tile.
     */
    public Tile t2 = null;
    ...
    /**
     * Writes the dictionary file(s) for this tile. Walks down the tile tree to
     * do this.
     *
     * @param ds Stream to use for writing if this is a container or route
     * container tile
     * @param deep Current depth of this tile
     * @param fid File ID to use
     * @param path Base path to use for the map data
     * @throws IOException if the file could not be created or if there was an
     * IO error
     */
...
//				lds.writeShort(fid);
    ...
    /**
     * Recalculate the bounds of this tile tree starting from the current tile
     * in case new tiles have been added to the tree. Leaf tiles are assumed to
     * have the right bound.
     *
...
//						}
    ...
    /**
     * Checks for coast lines and creates one (or more) sea polygons which
     * extend(s) to the boundaries of this tile. If this tile is a container, it
     * makes its children create these areas.
     */
    ...
    /**
     * Recalculates the idxMin and idxMax on RouteContainerTiles and
     * RouteDataTiles
     *
     * @return
     */
...
//			System.out.println("resolve T2 with " + idxMin + " to "+ idxMax);
        ...
        if (routeNodes != null) {
                        ...
                        for (Connection c : n.getConnected()) {
                            ...
                            for (int i = 0; i < TravelModes.travelModeCount; i++) {
                                ...
                                if ((c.connTravelModes & (1 << i)) != 0) {
                                    /**
                                     * If we can't fit the values into short, we
                                     * write an int. In order for the other side
                                     * to know if we wrote an int or a short, we
                                     * encode the length in the top most (sign)
                                     * bit
                                     */
                                    int time = c.times[i];
            ...
            /**
             * Write a special marker, so that we can detect if something went
             * wrong with decoding the variable length encoding
             */
