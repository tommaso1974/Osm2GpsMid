 ...
 */
/**
 * g // total cost h //heuristic f //sum of g and h;
 *
 *
 * Create a node containing the goal state node_goal Create a node containing
 * the start state node_start Put node_start on the open list while the OPEN
 * list is not empty { Get the node off the open list with the lowest f and call
 * it node_current if node_current is the same state as node_goal we have found
 * the solution; break from the while loop Generate each state node_successor
 * that can come after node_current for each node_successor of node_current {
 * Set the cost of node_successor to be the cost of node_current plus the cost
 * to get to node_successor from node_current find node_successor on the OPEN
 * list if node_successor is on the OPEN list but the existing one is as good or
 * better then discard this successor and continue if node_successor is on the
 * CLOSED list but the existing one is as good or better then discard this
 * successor and continue Remove occurrences of node_successor from OPEN and
 * CLOSED Set the parent of node_successor to node_current Set h to be the
 * estimated distance to node_goal (Using the heuristic function) Add
 * node_successor to the OPEN list } Add node_current to the CLOSED list }
 */
package osmToGpsMid;
