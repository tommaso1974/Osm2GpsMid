 ...
 *
 * Copyright (C) 2007 Harald Mueller Copyright (C) 2007, 2008 Kai Krueger
 * Copyright (C) 2008 sk750
 *
 */
package osmToGpsMid;

import static osmToGpsMid.GetText._;
...
public class CreateGpsMidData implements FilenameFilter {
    ...
    /**
     * This class is used in order to store a tuple on a dedicated stack. So
     * that it is not necessary to use the OS stack in recursion.
     */
    class TileTuple {
     ...
     * the route tile. Each one is actually a tree of tiles because container
     * tiles contain two child tiles.
     */
...
//		System.exit(2);
    ...
    private void exportLegend(String path) {
        ...
        try {
            ...
            /**
             * Note if additional information is included that can enable
             * editing of OSM data
             */
             ...
             */
            /**
             * // polish.api.bigstyles * Are there more way or poi styles than
             * 126
             */
            ...
            /**
             * write all sound files in each sound directory for all sound
             * formats
             */
    ...
    public void removeUnusedSoundFormats(String path) {
        ...
        if (sounds != null) {
            for (File file : sounds) {
                if (file.getName().matches(".*\\.amr") && soundMatch.indexOf(";amr;") == -1
...
//					System.out.println("realMediaPath not found: " + realMediaPath);
                    ...
                    if (CreateGpsMidData.class.getResource("/media/" + additionalSrcPath + "/" + mediaPath) == null) {
                        ...
                        /**
                         * Copy the file from Osm2GpsMid.jar to the destination
                         * directory
                         */
                        try {
...
//			System.out.println("Copying " + mediaPath + " as " + outputMediaName + " into the midlet");
    ...
    /**
     * Prepares and writes the tile's node and way data. It splits the tile,
     * creating two sub tiles, if necessary and continues to prepare and write
     * their data down the tree. For writing, it calls writeRenderTile() or
     * writeRouteTile().
     *
...
//								MAX_RAD_RANGE +	": " + t.bounds);
                ...
                /**
                 * If the number of nodes and ways in the new tile is the same,
                 * and the bound has already been shrunk to less than 0.001°,
                 * then give up and declare it a unsplittable tile and just live
                 * with the fact that this tile is too big. Otherwise we can get
                 * into an endless loop of trying to split up this tile.
                 */
    ...
    /**
     * Writes the byte array to a file for the file t i.e. the name of the file
     * is derived from the zoom level and fid of this tile. Also marks all ways
     * of t and sets the fid of these ways and of all nodes in 'nodes'. Plus it
     * sets the type of t to Tile.TYPE_MAP, sets its bounds to realBound and
     * updates totalNodesWritten, totalWaysWritten, totalSegsWritten and
     * totalPOIsWritten.
     *
...
//			" ways:" + t.ways.size() + " nodes:" + nodes.size());
    ...
    /**
     * Collects all ways from parentWays which 1) have a type >= 1 (whatever
     * that means) 2) belong to the zoom level zl 3) aren't already marked as
     * used and 4) are mostly inside the boundaries of targetBounds. realBound
     * is extended to cover all these ways.
     *
...
//		System.out.println("getWaysInBound found " + ways.size() + " ways");
     ...
     * Collects all ways from parentWays which 1) have a type >= 1 (whatever
     * that means) 2) belong to the zoom level zl 3) aren't already marked as
     * used 4) are completely inside the boundaries of targetTile. Ways are only
...
//		System.out.println("addWaysCompleteInBound found " + ways.size() + " ways");
    ...
    /**
     * Find all nodes out of the given collection that are within the bounds and
     * in the correct zoom level.
     *
...
//				node.used = true;
    ...
    /**
     * Create the data-content for a route-tile, containing a list of nodes and
     * a list of connections from each node.
     *
     * @param interestNodes list of all Nodes that should be included in this
     * tile
    ...
    /**
     * Create the Data-content for a SingleTile in memory. This will later
     * directly be written to disk if the byte array is not too big, otherwise
     * this tile will be split in smaller tiles.
     *
     ...
     * @return a byte array that represents a file content. This could be
    ...
    /**
     * Adds the node n and its ID to the map wayNodes if it's an unused node and
        ...
        for (Connection c : n.routeNode.getConnected()) {
            ...
            for (int i = 0; i < TravelModes.travelModeCount; i++) {
                ...
                if ((c.connTravelModes & (1 << i)) != 0) {
                    /**
                     * If we can't fit the values into short, we write an int.
                     * In order for the other side to know if we wrote an int or
                     * a short, we encode the length in the top most (sign) bit
                     */
                    int time = c.times[i];
    ...
    private void writeNode(Node n, DataOutputStream ds, int type, Tile t) throws IOException {
        ...
        /**
         * Convert coordinates to relative fixpoint (integer) coordinates The
         * reference point is the center of the tile. With 16bit shorts, this
         * should allow for tile sizes of about 65 km in width and with 1 m
         * accuracy at the equator.
         */
